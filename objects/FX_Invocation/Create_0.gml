...
var flash_default = 0.15 * game_fps;
flash_duration = (variable_instance_exists(self, "flash_duration_ms") ? max(1, (flash_duration_ms / 1000.0) * game_fps) : flash_default);
post_fx_t = 0;
finished_move = false;
...
ss_alpha = 0.9;
_ss_init_done = false;
ss_intro_frames = round(0.15 * game_fps);
ss_anim_total_frames = round(2.0 * game_fps);
ss_anim_t = 0;
ss_hold_total_frames = round(1.0 * game_fps);
ss_hold_t = 0;
ss_zoom_frames = round(0.5 * game_fps);
ss_anim_frames = round(1.0 * game_fps);
ss_move_total_frames = round(1.0 * game_fps);
...
if (snd_invocation != -1) {
    ...
    if (variable_instance_exists(self, "summon_mode") && summon_mode == "SpecialSummon") {
        var pre_total_local = (variable_instance_exists(self, "ss_pre_total_frames") ? ss_pre_total_frames : round(2.0 * game_fps));
        var move_total_local = (variable_instance_exists(self, "ss_move_total_frames") ? ss_move_total_frames : round(1.0 * game_fps));
        total_frames = pre_total_local + move_total_local + post_fx_duration + flash_duration;
    }
    var total_ms = max(1, round((total_frames / game_fps) * 1000));
    var snd_len_ms = 3000;
