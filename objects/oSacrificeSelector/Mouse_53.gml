...
// Si le GraveyardViewer est ouvert, bloquer les clics sur le SacrificeSelector
...
// Positions des boutons (synchro avec Draw_0 + marges internes)
// (Supprimé: ancien calcul basé sur frame extérieur)
...
// Vérifie si le clic est sur un monstre sur le terrain (sans bouton "+" dédié)
...
for(var i = 0; i < nbCards; i++) {
    ...
    if(card.type == "Monster" && card.zone == "Field" && card.isHeroOwner) {
        ...
        if(mouse_x >= card_left && mouse_x <= card_right && mouse_y >= card_top && mouse_y <= card_bottom) {
